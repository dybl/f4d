...
import com.asha.vr.plugins.MDPluginAdapter;
import com.asha.vr.strategy.ModeManager;

import java.util.LinkedList;
import java.util.List;

import static com.asha.vr.common.VRUtil.checkGLThread;
...
public class ProjectionModeManager extends ModeManager<AbsProjectionStrategy> implements IProjectionMode {
    ...
    public static int[] sModes = {MDvrrary.PROJECTION_MODE_SPHERE, MDvrrary.PROJECTION_MODE_DOME180, MDvrrary.PROJECTION_MODE_DOME230};
    ...
    public void on(Activity activity) {
        ...
        for (int i = 0; i < MDvrrary.sMultiScreenSize; i++){
    ...
    protected AbsProjectionStrategy createStrategy(int mode) {
        ...
        switch (mode){
            case MDvrrary.PROJECTION_MODE_DOME180:
                return new DomeProjection(this.mTextureSize,180f,false);
            case MDvrrary.PROJECTION_MODE_DOME230:
                return new DomeProjection(this.mTextureSize,230f,false);
            case MDvrrary.PROJECTION_MODE_DOME180_UPPER:
                return new DomeProjection(this.mTextureSize,180f,true);
            case MDvrrary.PROJECTION_MODE_DOME230_UPPER:
                return new DomeProjection(this.mTextureSize,230f,true);
            case MDvrrary.PROJECTION_MODE_STEREO_SPHERE_HORIZONTAL:
                return new StereoSphereProjection(MDDirection.HORIZONTAL);
            case MDvrrary.PROJECTION_MODE_STEREO_SPHERE:
            case MDvrrary.PROJECTION_MODE_STEREO_SPHERE_VERTICAL:
                return new StereoSphereProjection(MDDirection.VERTICAL);
            case MDvrrary.PROJECTION_MODE_PLANE_FIT:
            case MDvrrary.PROJECTION_MODE_PLANE_CROP:
            case MDvrrary.PROJECTION_MODE_PLANE_FULL:
                return PlaneProjection.create(mode,this.mTextureSize);
            case MDvrrary.PROJECTION_MODE_MULTI_FISH_EYE_HORIZONTAL:
                return new MultiFishEyeProjection(1f, MDDirection.HORIZONTAL);
            case MDvrrary.PROJECTION_MODE_MULTI_FISH_EYE_VERTICAL:
                return new MultiFishEyeProjection(1f, MDDirection.VERTICAL);
            case MDvrrary.PROJECTION_MODE_SPHERE:
            default:
